...
CharacterSelectP2::CharacterSelectP2(GameDataRef data, int x, int y, std::string title, int player1Choice) : _data(data), _selectedShipIndex(0) {}

void CharacterSelectP2::Init()
{

    // Load the background texture
    _data->assets.LoadTexture("Character Select Screen Background", CHARACTER_SELECT_BACKGROUND_FILEPATH_P2);
    ...
    _Goliath.setFillColor(sf::Color::Red); // Highlighted color
...
void CharacterSelectP2::HandleInput()
{
    sf::Event event;
    while (_data->window.pollEvent(event))
    {
        ...
        if (event.type == sf::Event::KeyPressed)
        {
            ...
            else if (event.key.code == sf::Keyboard::Enter || event.key.code == sf::Keyboard::Enter)
            {
                SelectItem();
            }
        }
...
void CharacterSelectP2::Update(float frameRate)
{
    // Update logic if any
}

void CharacterSelectP2::Draw(float frameRate)
{
    _data->window.clear();
...
void CharacterSelectP2::MoveLeft()
{
    if (_selectedShipIndex > 0)
    {
        ...
        if (_selectedShipIndex == 0)
        {
            _Goliath.setFillColor(sf::Color::Red);
            _Arachne.setFillColor(sf::Color::White);
            _Flea.setFillColor(sf::Color::White);
        }
        else if (_selectedShipIndex == 1)
        {
            ...
            _Arachne.setFillColor(sf::Color::Red);
            _Flea.setFillColor(sf::Color::White);
        }
        else if (_selectedShipIndex == 2)
        {
            _Goliath.setFillColor(sf::Color::White);
            _Arachne.setFillColor(sf::Color::White);
            _Flea.setFillColor(sf::Color::Red);
        }
    }
}

void CharacterSelectP2::MoveRight()
{
...
void CharacterSelectP2::SelectItem()
{
    ...
    if (_selectedShipIndex == 0)
    {
        ...
        _data->machine.AddState(StateRef(new laserFighters2000(_data, _data->window.getSize().x, _data->window.getSize().y, "Laser Fighters 2000", player1Choice, _selectedShipIndex)), true);
    }
    else if (_selectedShipIndex == 1)
    {
        // Transition to the game state
